...
int numeroHojas(AB T)
{
    ...
    if (T->izq == NULL && T->der == NULL)
        return 1;
    return numeroHojas(T->izq) + numeroHojas(T->der);
}

///CREA AQUÍ LA FUNCIÓN "numeroRepeticiones" QUE RETORNE CUÁNTAS VECES "se repite un dato" DENTRO DE UN ÁRBOL (Ejercicio 6) ///

int numeroRepeticiones(AB T, int dato)
{
    if (T == NULL)
        return 0;
    if (T->info == dato)
        return 1 + numeroRepeticiones(T->izq, dato) + numeroRepeticiones(T->der, dato);
    else
        return numeroRepeticiones(T->izq, dato) + numeroRepeticiones(T->der, dato);
}
